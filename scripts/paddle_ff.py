#!/usr/bin/env python3
"""PaddleOCR-VL full-form benchmark (CER/WER + IoU + reading order)."""
import json, re, sys, time
from datetime import datetime, timezone
from pathlib import Path

IMAGES_DIR = Path("/data/curated")
GT_PATH = Path("/data/ground_truth.json")
RESULTS_DIR = Path("/results")
CANDIDATE = "paddleocr_vl_fullform"

# --- metrics (same as paddle_hw.py) ------------------------------------
def _ed(s1, s2):
...
def cer(r,h): return _ed(_norm(r),_norm(h))/len(_norm(r)) if _norm(r) else (1.0 if _norm(h) else 0.0)
...
def main():
    ...
    result={
        ...
        "notes": "PaddleOCR-VL-1.6 full-form benchmark. Docker sm120-offline."
    }
